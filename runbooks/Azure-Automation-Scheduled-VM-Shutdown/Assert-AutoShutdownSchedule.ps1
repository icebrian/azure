<#
    ...
    .DESCRIPTION
       ...
       e.g. "10PM -> 6AM". It then checks the current time against each schedule entry, ensuring that VMs with tags or in tagged groups
        ...
        ie: "AutoShutdownSchedule: 11PM -> 6AM, Saturday, Sunday"

        Based on: https://automys.com/library/asset/scheduled-virtual-machine-shutdown-startup-microsoft-azure
...
$SubId = Get-AutomationVariable -Name 'Default Azure Subscription'
try
{
...
Write-Output "Specified credential name: [$AzureCredentialAssetName]"

$azureCredential = Get-AutomationPSCredential -Name $AzureCredentialAssetName -ErrorAction SilentlyContinue
$account = Add-AzureAccount -Credential $azureCredential -ErrorAction Stop -ErrorVariable err
$null = Add-AzureRmAccount -Credential $azureCredential -ErrorAction Stop -ErrorVariable err
